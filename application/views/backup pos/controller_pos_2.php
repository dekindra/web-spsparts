...
class Pos extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // $data['tax'] = $setting_tax; //hapuskan segera
        $data['display_prod'] = $setting_image;
        $data['keyboard'] = $settingData->display_keyboard; // tidak apa apa ini
    ...
    // Get Product Detail;
    public function getProductDetail() //sudah dicek aman
    {
        $pcode = $this->input->get('pcode');
        $outlet_id = $this->input->get('outlet_id');

        $pcodeDtaData = $this->Constant_model->getDataOneColumn('produk', 'code_p', $pcode);

        $prod_name = $pcodeDtaData[0]->name_p;
        $prod_price = $pcodeDtaData[0]->purchase_price; //ini belom bener baru pengecekan tok

        // Check Inv;
        $qty = 0;
        $ckInvData = $this->Constant_model->getDataOneJoinTwoWhere('inventory','id_p', 'produk', 'id_p', 'code_p', $pcode, 'id_bengkel', $outlet_id);
        // $ckInvData = $this->Constant_model->getDataTwoColumn('inventory', 'product_code', $pcode, 'outlet_id', $outlet_id);
        if (count($ckInvData) > 0) {
            $qty = $ckInvData[0]->stock;
        }
    ...
    // Send Email;
    public function send_invoice() //sudah sedikit yg tak edit tapi belum selesai
    {
        $settingResult = $this->db->get_where('site_setting');
        $settingData = $settingResult->row();
        ...
        $orderData = $this->Constant_model->getDataOneColumn('order', 'id', $id);
        $ordered_dtm = date('Y-m-d H:i:s', strtotime($orderData[0]->ordered_datetime));
...
<div id="wrapper">
    ...
    public function addNewCustomer() //sudah dicek aman
    {
        ...
        $this->Constant_model->insertData('pelanggan', $ins_data);
    ...
    public function loadCustomer() //sudah dicek aman
    {
        $settingResult = $this->db->get_where('site_setting');
        $settingData = $settingResult->row();
        ...
        $getLastCustResult = $this->db->query('SELECT * FROM pelanggan ORDER BY id DESC LIMIT 0,1');
        ...
        if ($minutes <= 10) {
            $custData = $this->Constant_model->getDataAll('pelanggan', 'id', 'DESC');
        } else {
            $custResult = $this->db->query("select * from pelanggan order by case when id = '$site_default_cust_id' then 1 else 2 end ");
    ...
    public function loadGiftCardValue() //sudah dicek == aman
    {
        $card_numb = $this->input->get('card_numb');

        $today = date('Y-m-d', time());

        $ckgiftResult = $this->db->query("SELECT * FROM promo WHERE card_number = '$card_numb' ");
        $ckgiftRows = $ckgiftResult->num_rows();
        if ($ckgiftRows == 1) {
            $ckgiftData = $ckgiftResult->result();
    ...
    public function loadTodaySales() //sudah dicek == aman akan tetapi database nya belom dibuat
    {
        $outlet_id = $this->input->get('outlet_id');
        ...
        $orderResult = $this->db->query("SELECT * FROM penjualan WHERE created_datetime >= '$today_start' AND created_datetime <= '$today_end' AND outlet_id = '$outlet_id' ");
